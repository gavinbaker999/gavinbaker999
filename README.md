...
Using the Raspberry PI Zero and Pico models to develop automation, robotic and data collection projects. These include a robot that can sense its environment and a weather data collection station. Other services used include PushOver/Magic Bell for notifications. A Raspbery PI 400 which runs mainly as a media server (Plex) and backup server (Samba) on my home lab network, along with a Pi Zero which is my Linix SSH box.
